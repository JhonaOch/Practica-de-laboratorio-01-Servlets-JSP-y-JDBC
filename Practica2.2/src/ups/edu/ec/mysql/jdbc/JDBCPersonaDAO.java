...
public class JDBCPersonaDAO extends JDBCGenericDAO<Persona, String> implements PersonaDAO {
    ...
    public boolean create(Persona persona) {
        return conexionUno.update("INSERT INTO usuario VALUES ('" + persona.getCedula() + "', '" + persona.getNombres() + "', '" + persona.getApellidos() + "', '" + persona.getCorreo() + "', '" + persona.getContrasena() + "'," + persona.getActivo() + ");");
    }
    ...
    public Persona findById(String cedula) {
        ...
        try {
            if (rs != null && rs.next()) {
                persona = new Persona(rs.getString("usu_cedula"), rs.getString("usu_nombres"), rs.getString("usu_apellidos"), rs.getNString("usu_correo"), rs.getNString("usu_contrasena"), rs.getInt("usu_activo"));
    ...
    public boolean update(Persona persona) {
        return conexionUno.update("UPDATE usuario SET "
                + "	usu_nombres = '" + persona.getNombres() + "',"
                + "	usu_apellidos = '" + persona.getApellidos() + "',"
                + "	usu_contrasena  = '" + persona.getContrasena() + "'"
    ...
    public Persona findUser(String correo, String contrasena) {
       ...
       Persona persona = null;
        ...
        try {
            if (rs != null && rs.next()) {
                persona = new Persona(rs.getString("usu_cedula"), rs.getString("usu_nombres"), rs.getNString("usu_apellidos"), rs.getNString("usu_correo"), rs.getNString("usu_contrasena"), rs.getInt("usu_activo"));
                List<Telefono> telefonos = DAOFactory.getDAOFactory().getTelefonoDAO().findByUserId(persona.getCedula());
                persona.setLista(telefonos);
            }
        } catch (SQLException e) {
